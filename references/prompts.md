...
across all scene stills 鈥?that identical text is what makes the world feel like one place.

## Intake checklist (Step 1)

Default render path: WaveSpeedAI. Before collecting creative choices, run
`references/bootstrap.py`; if it asks for `WAVESPEED_API_KEY`, ask the user for that key
and verify it through the bootstrap. Monid, Higgsfield CLI, and Codex CLI are optional
fallbacks only and should not be installed or required for the default path.

Collect and write down:

- `SUBJECT` 鈥?the business + one-line pitch.
- `BRAND_NAME` 鈥?display name.
- `PALETTE` 鈥?4鈥? named hexes, e.g. `taro #9B7EBD, cream #F5EDE0, caramel #C88A5A, matcha #8FB98A, plum #3A2E48`. Pick ONE as the scene **background** colour (usually the lightest) and one as the primary **accent**.
- `TONE` 鈥?a word or two (cozy/premium, playful, industrial鈥?.
- `STYLE` 鈥?the art direction (default below).
- `SECTIONS[]` 鈥?ordered list; for each: `id`, `label`, `subject` (what's in the diorama), `eyebrow`, `title`, `body` (鈮?1 sentence), `tags[]` (0鈥?). Last section = hero product + CTA.
- `CAMERA` 鈥?fly-through (arch B: dives + aerial hops 鈥?the flagship-demo look) |
...
- `MOBILE` 鈥?yes/no. **Always asked** (SKILL Step 1.6), presented to the user
  with the ~2脳 credit cost stated. Yes = the **native 9:16 portrait chain**
  (pipeline 搂6b): portrait renders of every dive/connector +
  `clipMobile`/`connectorsMobile`/`stillMobile` wiring + the full mobile QA. The
  搂6 crop encodes are a no-credits stopgap only.
- `VIDEO_TIER` 鈥?default is WaveSpeedAI `bytedance/seedance-2.0` (pay-per-clip; previz =
  same endpoint at 480p, final at 1080p). Higgsfield/Monid fallbacks: draft
  ...
  anything renders.
- `STILLS_SOURCE` 鈥?higgsfield (`gpt_image_2`, spends credits) | codex
...
- **Compose for the centre.** The page renders every clip `object-fit:cover`. Keep the
  ...
  (pipeline 搂6b), so this is not about surviving a crop 鈥?but a centred composition makes
...
## Leg prompt 鈥?architecture A, continuous forward take (Step 4)

`--start-image = previous leg's ACTUAL last frame` (leg 0: the first scene's still).
**No `--end-image`.** The bolded clauses are the motion-handoff contract 鈥?keep them
...
forward glide.** [MID-LEG MOVE 鈥?optional, from the library below.] The camera moves
...
Reversals are safe *inside* a leg (it's one continuous render) 鈥?only a seam may never
reverse. That's why "ease back out" is fine mid-leg.

**Locked-iso clause** (`CAMERA` = locked isometric glide 鈥?SKILL Step 1.4/Step 4):
skip this library entirely and put this clause, verbatim, in the mid-leg slot of
EVERY leg:

```
The camera keeps exactly the same high isometric angle throughout 鈥?no rotation, no
...
frame, also check the ANGLE hasn't drifted (Seedance rotates slightly on long legs) 鈥?re-roll the leg if the view has turned.
...
orbit). If it doesn't, re-roll this leg 鈥?a bad handoff frame poisons every leg after it.
...
and descends toward it, sweeping in toward [FOCAL POINT 鈥?the counter/the cauldrons/the
...
Params by chain model (SKILL Step 4 table): seedance 鈥?`--mode std --resolution 1080p --aspect_ratio 16:9 --duration 8`, no audio flag;
kling3_0 鈥?`--mode std --sound off --aspect_ratio 16:9 --duration 10` (no `--resolution`
...
For the last connector into a hero-product finale: "鈥lides forward and the world
...
鈫?use a roster model that accepts it (Step 4).

## Copy per section (for the engine config)

- `eyebrow` 鈥?2鈥? words, uppercase feel (a value-prop label).
- `title` 鈥?3鈥? words, the beat's headline. First section = the site's hero line; last =
  the payoff + it carries the CTA.
- `body` 鈥?one sentence, plain-spoken, from the visitor's side.
- `tags` 鈥?0鈥? short proof chips (e.g. "Fresh-cooked", "30-min delivery").
