---
name: scroll-world-wavespeed
description: >
  Build an immersive scroll-scrubbed "fly through the world" landing page for any
  industry or brand. As the visitor scrolls, a pre-rendered camera flies from
  outside each scene into its interior, then flows on to the next scene with NO
  cuts - one continuous connected flight (Emons-style isometric diorama world, or
  any art direction you pick). The skill interviews the user for the topic, story
  beats/sections, and brand kit, then generates cohesive scenes + seamless camera
  clips through WaveSpeedAI by default and wires a portable, framework-agnostic
  scroll-scrub engine. Monid, Higgsfield CLI, and Codex CLI paths are optional
  fallbacks only.
...
**Current default path (read first):** Use WaveSpeedAI by default for both stills and
video: `openai/gpt-image-2` for scene stills and `bytedance/seedance-2.0` for chained
clips. Monid, Higgsfield CLI, and Codex CLI are optional fallback paths only; do not ask
default-path users to install them. The only hard local runtime dependency for the
default path is `ffmpeg`/`ffprobe` (plus `curl` and `python3`/`python`, which are checked
by the bootstrap).
...
cuts. The visuals are AI-generated through **WaveSpeedAI by default**:
`openai/gpt-image-2` for stills and `bytedance/seedance-2.0` for video. Monid,
Higgsfield CLI, and Codex CLI are optional fallback paths only. The page scrubs
pre-rendered video by scroll position; the camera genuinely moves and scroll only
drives time.


**What you generate:** N scene stills 鈫?N "dive-in" camera clips 鈫?N-1 "connector" clips
that join consecutive scenes seamlessly 鈫?a portable scrub engine that plays the whole
chain as one flight.
...
The value of this skill is the WaveSpeed-first pipeline, the prompts, and the seam method - not the framework.

---

## Step 0 鈥?Bootstrap

0. **Run the default bootstrap first.** From the skill root, run
   `python3 references/bootstrap.py` (or `python references/bootstrap.py` on Windows).
   It checks `curl`, `python3`/`python`, and `ffmpeg`/`ffprobe`; installs ffmpeg when a
   known package manager is available (`winget`, `choco`, `scoop`, Homebrew, apt, dnf,
   pacman, apk); creates `.env`; and verifies `WAVESPEED_API_KEY` against
   `https://api.wavespeed.ai/api/v3/balance`. The correct host is
   `api.wavespeed.ai`; never use `api.wavespeedai.com`.

   If the bootstrap reports that `WAVESPEED_API_KEY` is missing, ask the user for it
   immediately, then rerun with `WAVESPEED_API_KEY=<key> python3 references/bootstrap.py`.
   Never print the key back to the user, commit `.env`, or write it into generated
   public project files. After bootstrap succeeds, load it before generation:
   `set -a; source .env; set +a`.

   The bootstrap is part of the skill contract. Future users should not have to discover
   missing ffmpeg manually, and they should not be sent to install Monid/Higgsfield/Codex
   for the default WaveSpeed path.

1. **Backend + billing 鈥?pick before anything else, and ask which audience this
   build is for if it isn't obvious.** Two supported paths, both roster-qualified
   (Step 4). **WaveSpeedAI is the default/primary path for this skill** 鈥?reach for
   Monid + Higgsfield only when the build genuinely needs it (international/USD
   billing, or a Higgsfield-only model like `kling3_0`):
   - **WaveSpeedAI 鈥?the default path, incl. mainland-China audiences** (pipeline.md
     搂7): needs only `curl`, `ffmpeg`/`ffprobe`, and `python3` (all near-universal 鈥?     no CLI install, no OAuth login, no VPN to reach the API or to pay: card or RMB
     via Alipay/WeChat) plus a `WAVESPEED_API_KEY`. `jq` is used if present but is
     **not required** 鈥?pipeline.md 搂7's helpers auto-fall-back to a bundled python3
     JSON parser, so a minimal sandbox missing `jq` (common on hosted agent
     platforms) still works. Copy the skill's `.env.example` to `.env` (gitignored)
     and have the user drop their own key in; `source .env` before running any
     `gen_*_ws` function. One model (`bytedance/seedance-2.0`) covers stills, dives,
     *and* connectors 鈥?Higgsfield/Monid are not needed on this path, full stop; do
     not tell a user on this path they need to install either CLI.
   - **Monid + Higgsfield 鈥?alternate path, international/USD builds only**
     (pipeline.md 搂7b): check `monid --version`, `monid keys list`, `monid balance`
     (billed per clip in USD; Step 1.7 has the numbers; a 1080p N=6 chain 鈮?$27).
     Higgsfield CLI is required on this path (see bullet 2) 鈥?but *only* on this
     path.
2. **Higgsfield CLI 鈥?only required on the Monid path (skip entirely on WaveSpeed).**
   It renders the scene stills (`gpt_image_2`) and is the home of the `kling3_0`
   NSFW fallback and the fallback chain. If `higgsfield` is not on `$PATH`, install
   per the `higgsfield-generate` skill. If `higgsfield workspace list` fails auth,
   ask the user to run `higgsfield auth login` (interactive OAuth 鈥?you cannot run
   it) and, if needed, `higgsfield workspace set <id>`. Confirm credits cover the
   stills (~N image gens) 鈥?plus `(2N-1)` video gens if the chain falls back here.
3. **ffmpeg / ffprobe** on `$PATH` (frame extraction + encoding) 鈥?required on
   every path, including WaveSpeed.
4. **An image tool** for background knockout if you want floating scenes: PIL
   (`python3 -c "import PIL"`), or `cwebp`/`sips`. Optional 鈥?see Step 3.
5. **(Optional) Codex CLI** 鈥?if `codex` is on `$PATH` (鈮?0.125) and
   ...
   user's ChatGPT subscription instead of Higgsfield credits 鈥?offer it at
...
6. Caveats: macOS ships **bash 3.2** (no `declare -A`); don't use associative arrays in
   scripts. Higgsfield generations take **3鈥? min each** 鈥?always run them detached
   (background) and poll, never a foreground blocking call. Reference-by-job-UUID is
   rejected by media flags 鈥?pass **local file paths** to `--image/--start-image/--end-image`.
   Video models differ in accepted params (e.g. Kling has no `--resolution`) and in whether
   they support start/end-image conditioning at all 鈥?before batching, confirm the chosen
   model's schema with `higgsfield model get <job_type>` and see the Step 4 model table.

---

## Step 1 鈥?Interview the user

The **subject is the user's to state 鈥?ask it as an open question in plain prose**, never a
...
enumerable, lower-stakes choices below 鈥?art direction, camera style, and brand-kit
approach 鈥?and even
there, signal they can go their own way ("Other"). Ask only what you can't sensibly
default. Cover:

1. **Subject** (ask openly, not multiple-choice) 鈥?"What should this world be about? Your
   business, a client's, or any idea 鈥?a word or a sentence is fine." Capture the
...
2. **Brand kit** 鈥?offer three paths, pick one:
   ...
   Capture **4鈥? named hex values**, a display name, and a tone word or two.
3. **Art direction** 鈥?default is "soft matte low-poly **clay diorama**, isometric,
...
4. **Camera style 鈥?ALWAYS ask; it's the film's personality, not a technical
   ...
   architectures 鈥?Step 4 then *implements* the choice, it never re-decides it:
   - **"Fly through the world"** 鈥?the camera dives into each scene, pulls up and
     ...
     鈫?Architecture B. Recommend as the default for diorama/miniature art
     directions.
   - **"One continuous walkthrough"** 鈥?a single forward flight that glides
     through each scene straight into the next, never pulling back; expressive
     but always-forward moves per scene (camera grammar table). 鈫?Architecture A.
     Recommend as the default for grounded/photoreal art directions.
   - **"Locked isometric glide"** 鈥?the camera keeps one fixed angle for the whole
     film, Emons-style; the world slides past/toward it, no rotation, no reveals.
     鈫?Architecture A + the locked-iso clause in every leg prompt (prompts.md).
   State the trade-off in one line each (B reverses direction at seams 鈥?charming
   in miniature, jarring in realism; locked-iso is the calmest and cheapest to
   re-roll; walkthrough sits between).
5. **The journey (sections)** 鈥?the ordered scenes the camera flies through. Propose a
   set derived from the subject's own value chain and let the user edit. 5鈥? works well.
   Boba example: farms 鈫?pearl kitchen 鈫?flagship shop 鈫?delivery 鈫?community plaza 鈫?   the hero product. Each section needs: a short subject description (what's IN the
   diorama), an eyebrow, a headline, one line of body, and 0鈥? tag pills. The last
   section is usually the hero product + the CTA.
6. **Mobile version 鈥?ALWAYS ask this; never silently generate both.** Ask as a
   ...
   *"Want a mobile-optimized version too? The mobile version is a second camera chain
   rendered natively in **9:16 portrait** 鈥?composed for phones, not a crop of the
   landscape film 鈥?which roughly doubles the Higgsfield credit spend (state the
   estimated number)."*
   Options: "Desktop only" / "Desktop + mobile (native 9:16 鈥?~2脳 credits)". The
   credit cost must be stated to the user, not just implied.
   What the answer gates:
   - **Yes** 鈫?render the parallel 9:16 portrait chain and ship it as the mobile variants
     (Step 6 / pipeline.md 搂6b): portrait start canvases 鈫?9:16 dives + connectors
     frame-locked against their own renders 鈫?720-wide `-m.mp4` encodes 鈫?`stillMobile`
     ...
     **Never ship the centre-crop as the mobile version by default** 鈥?if credits can't
     cover the portrait chain, say so and offer the crop encodes (pipeline.md 搂6) as an
     explicitly-labelled stopgap the user must approve.
   - **No** 鈫?skip the mobile encodes and wiring entirely. The engine's phone hardening
     (seek-coalescing, iOS priming, safe-area CSS) is always on regardless 鈥?that's not
     a "mobile version," it's just the page not breaking when a phone visits 鈥?so a
     desktop-only build still degrades gracefully.

7. **Budget 鈥?engines shown by cost, decided before anything renders.** Present the
   render tiers (`AskUserQuestion`), then compute and state the estimated total for
   the user's N scenes 鈥?`N stills + (2N鈭?) videos [videos 脳2 if mobile] + ~15%
   re-roll headroom` 鈥?and get a go before generating.
   - **Video tier** (roster only 鈥?every option frame-locks seams, Step 4):

     | Tier | Model | Rough cost |
     |---|---|---|
     | Draft / previz | `seedance_2_0_mini` (720p) | ~录 of Standard |
     | Standard (default) | `seedance_2_0` (1080p) | baseline |
     | Alternate | `kling3_0` (720p native) | 鈮?Standard; different look + content filter |

     Draft doubles as the previz path: run the whole chain cheap, approve the
     journey, re-render final legs on Standard (pipeline.md Notes) 鈥?suggest it
     unprompted when the balance reads tight.
   - **Backend - WaveSpeedAI is the DEFAULT biller for the chain** (Step 0; pipeline.md section 7).
     Use `openai/gpt-image-2` for stills and `bytedance/seedance-2.0` for all chained
     video clips. Previz uses the same WaveSpeed endpoint at `VRES=480p`; final defaults
     to `VRES=1080p`. Check balance with the bootstrap or:
     `curl -s https://api.wavespeed.ai/api/v3/balance -H "Authorization: Bearer $WAVESPEED_API_KEY"`.
     Run one still and one short video first, compare the before/after balance, then
     extrapolate before batching the rest. Monid/Higgsfield are fallbacks only when the
     user explicitly prefers them, WaveSpeed is down, or a Higgsfield-only model such as
     `kling3_0` is required.
   - **Stills source** (only offer if the Codex CLI is present, Step 0.5):
     Higgsfield `gpt_image_2` (spends credits) vs **Codex `image_gen`** 鈥?the same
     gpt-image-2 model billed to the ChatGPT subscription (zero credits; counts
     toward Codex usage limits; 1536脳1024 output 鈥?exactly 3:2, slightly under
     ...
     for all N stills of a build** 鈥?the two render with slightly different
   ...
   - **Calibrate costs, don't guess.** The CLI exposes no pricing and plans differ.
     ...
     video 鈮?40鈥?5 credits, still 鈮?15.) A real `not_enough_credits` mid-run is
     recoverable (finished clips survive; resume after top-up) but ugly 鈥?the
...
can't frame-lock is declined with a one-line why, not substituted in 鈥?use a roster
model instead.

Keep the scroll mechanic fixed (continuous fly-through) 鈥?that's the point of the skill.
See `references/prompts.md` for the intake checklist and copy structure.

---

## Step 2 鈥?Generate the scene stills
...
the brief is character/cartoon-heavy (note: `nano_banana_2` is a CLI alias 鈥?it resolves
...
- **Codex stills variant** (if chosen at Step 1.7 鈥?subscription-billed, zero
  ...
  ~1鈥? min per image; run a few in parallel, not all N at once 鈥?and keep the
  `< /dev/null`: parallel `codex exec` calls sharing a script's stdin hang
  waiting for input (Gotchas). Output lands at
  1536脳1024 (3:2) 鈥?fine for `--start-image` and posters. Everything downstream
  (cohesion review, knockout, dives) is unchanged.
- A generation may fail transiently (HTTP 503) 鈥?re-roll that one individually; don't
  restart the batch.
...
## Step 3 鈥?(Optional) Float the scenes
...
`references/knockout.py` (border-connected flood fill 鈥?preserves interior colour that
...
## Step 4 鈥?Camera architecture (implements the Step 1.4 choice)

How the camera moves *between* scenes is the single biggest quality lever. The user
already chose the style at the interview (`CAMERA`, Step 1.4): fly-through 鈫?**B**,
walkthrough 鈫?**A**, locked-iso 鈫?**A + the locked-iso leg clause** (prompts.md). If
the interview somehow skipped it, ask now 鈥?never silently pick for them. The two
shapes, and the grammar that colors them:

### Video model 鈥?pick ONE for the whole chain
...
need `--end-image`. That capability 鈥?not preference 鈥?is the selection rule. Check any
...
|---|---|---|
| `seedance_2_0` (default) | 鉁?/ 鉁?| Full chain (legs + connectors). `--mode std --resolution 1080p`. Its NSFW filter is the touchy one (see Gotchas). |
| `kling3_0` | 鉁?/ 鉁?| Full chain 鈥?tested: `--mode std --sound off --duration 5` with start+end images accepted, seams frame-lock cleanly. **No `--resolution` param** (don't pass one; `--mode std` returns **720p native** 鈥?encode what ffprobe reports, never upscale). Sound defaults **on** 鈫?`--sound off`. `--duration` default 5, try 10 for legs. Different content filter than Seedance 鈥?the sanctioned NSFW fallback. |
| `seedance_2_0_mini` | 鉁?/ 鉁?| Cheap draft tier that keeps frame-locking (720p). The previz tier: run the whole chain here first, then re-render final legs on the full model 鈥?still seamless, so it translates directly. |

Those three are the roster 鈥?all do both architectures. (`kling3_0_turbo` also frame-locks
via `--start-image`, but has no `--end-image`, so it's architecture-A-only and can't make
connectors; it also takes a different flag set 鈥?no `--mode`, has `--resolution` 鈥?so it
...
probe: **`minimax_hailuo`** (Hailuo-2.3, ~6 credits per 768p/6s clip vs 22鈥?2 for the
roster). Verified 2026-07: `--start-image` + prompt frame-locks (output frame 0 鈮?input,
PSNR 33 dB) and a forward-glide prompt was obeyed, gently. Constraints: the 2.3 variant
rejects `end_image` (no connectors 鈫?arch A only), output aspect follows the input image
(hand it a 16:9 canvas, not a bare 3:2 still), motion runs subtler than seedance, and
don't pass `--resolution` (the CLI mis-types the enum; the 768 default works 鈥?1080
supports 6s only). One clip 鈮?a chain: qualify a leg-to-leg handoff before betting a
full build on it.
...
- **One model for all chained clips.** Each renderer has its own motion/color/grain
  ...
  the NSFW fallback for a single stubborn clip (Gotchas) 鈥?a slight character shift on
  one 5s connector beats a missing connector.
- Default to `bytedance/seedance-2.0`, rendered through **WaveSpeedAI by default** (per-clip USD 鈥?  next section) with Higgsfield credits as the fallback biller (Step 0.1/1.6); honor
  a user's stated preference **only if the model qualifies** (frame-locking). If it
  doesn't, say so and use a supported model 鈥?never ship a non-seamless build to
...
### Monid backend - optional USD fallback (qualified 2026-07-25)

WaveSpeedAI's **`bytedance/seedance-2.0`** is the default API path. Monid's equivalent endpoint passed paid probes on
2026-07-25 and remains an optional fallback way this skill can render the chain, for **both
architectures** 鈥?it is the roster's `seedance_2_0` served pay-per-USD (wiring in
pipeline.md 鈫?"Monid backend"; Higgsfield renders the chain only as the fallback
biller or for Higgsfield-only models):

- **Leg probe** (prompt + `first_frame` image): output frame 0 鈮?input still
...
- **Connector probe** (prompt + `first_frame` + `last_frame`): start locked
  ...
  composition, prop-level drift) 鈥?the exact end-image behavior Seedance shows
...
The I/O contract differs from the Higgsfield CLI 鈥?three rules:

1. **Images go by URL, never inline.** `content` items are
   `{"type":"image_url","image_url":{"url":鈥,"role":"first_frame"|"last_frame"}`;
   ...
   file system: `sfs /put` 鈫?`curl -T` the bytes 鈫?`sfs /cat` returns a signed
   public URL to paste into the body ($0, explicitly built for this).
2. **Pass `ratio` explicitly** (`16:9`, or `9:16` for the mobile chain) 鈥?the
   adaptive default follows the input image's aspect instead.
3. **Bill-check every clip**: cost is token-priced
   (`w 脳 h 脳 24 脳 sec / 1024` at $7鈥?.7/1M); read `cost.value` off each run.

History that shaped these rules (still true as of 2026-07-25): the seedance
endpoints were text-to-video-only until late July 2026 鈥?**re-`inspect` before
...
cheap 480p clip): (1) prompt + first-frame from a real still 鈥?frame 0 must
match the input to codec noise (PSNR 鈮?30 dB) and `cost.value` must match the
advertised cell; (2) for connector duty, add a `last_frame` from a different
still 鈥?the end must land on that composition (Seedance-style near-miss is fine,
the crossfade covers it). Pass 鈫?pay-per-clip tier (arch A if start-only; full
roster if start+end).

### A) Continuous forward take 鈥?RECOMMENDED for grounded / realistic / walkthrough
...
(or an expressive mid-leg move under the motion-handoff contract 鈥?see **Camera grammar**
below), and **no `--end-image`** 鈥?an end-image of a wide establishing shot forces the
...
**no connectors** (skip Step 5) 鈥?the legs ARE the journey. Wire each leg as a section
...
### B) Dive-in + aerial connector 鈥?only for diorama / miniature / god's-eye worlds
...
鈫?backward pull-out). In a miniature/diorama world that reads as an intentional "zoom out
to the map, fly to the next island"; in a grounded first-person walkthrough it reads as a
jarring **rewind/stutter**. Use B only for the map-like aesthetic 鈥?which is exactly
...
### Camera grammar 鈥?the move should fit the concept (A is NOT "forward only")
...
- **Velocity continuity** at a seam means the camera must never *reverse across a seam* 鈥?  that's the rewind stutter.
- **Inside a single leg the camera is free.** One leg is one continuous render 鈥?there is
...
| Playful miniature (arch. B) | dives + aerial hops 鈥?the connector IS the grammar |

Honest costs: expressive mid-leg moves raise re-roll odds 鈥?the model can end a fancy move
...
like a frame from a gentle forward glide 鈥?if not, re-roll before wasting the next leg);
budget ~1 extra re-roll per expressive leg. A plain forward glide stays the zero-risk
default 鈥?use it for legs where the scene itself is the show.

**Locked-iso variant** (`CAMERA` = locked isometric glide): architecture A where every
leg pins the view instead of taking a mid-leg move 鈥?"the camera keeps exactly the same
...
long legs 鈥?the existing eyeball-each-last-frame rule is the catch; re-roll a leg whose
...
*clip* and restraint in the *scrub mapping* 鈥?they compound.

And remember scroll is a scrubber: visitors can scroll **up**, so every move also plays in
reverse. That's free and expected 鈥?no extra work 鈥?but it's another reason seam velocity
...
- Prompt: "Single continuous cinematic camera move, no cuts. Begin high and far looking
  at the whole <scene> from outside 鈥?descend and fly inside toward <focal point> 鈥?the
...
- Params (seedance): `--mode std --resolution 1080p --aspect_ratio 16:9 --duration 8`.
  ...
  Do **not** pass `--generate-audio` (it errors on seedance; audio is wasted anyway 鈥?  you'll mute).
- Run concurrently, detached, then download each `.result_url`. Re-roll individual
  failures. Keep the raw 1080p sources 鈥?you need their frames next.

---

## Step 5 鈥?Connectors (architecture B only)

Skip this whole step for architecture **A** 鈥?the forward take has no connectors; its legs
...
endpoints must be the ACTUAL RENDERED FRAMES of the neighbouring clips 鈥?never the
...
model must accept it 鈥?any roster model does (`seedance_2_0`, `seedance_2_0_mini`,
`kling3_0`):
...
## Step 6 鈥?Encode for smooth scrubbing
...
1. **Seekability, not keyframe density, is what makes scrubbing work.** Many static
   ...
   `video.seekable` to `[0,0]` and clamps *every* seek to frame 0 鈥?the video looks
...
2. **Don't shrink quality to get smooth seeks.** Encode at the **native resolution**
   (1080p from Seedance 鈥?don't downscale), `crf ~20`, a **small GOP** (`-g 8`) rather
...
the **native 9:16 portrait chain** (pipeline.md 搂6b): portrait renders of every dive and
connector, encoded **720 wide (`scale=720:-2`), `-g 4`** (more keyframes = cheaper seeks 鈥?phone decoders' seek cost scales with GOP length), crf 23 鈥?wired as `clipMobile` /
...
(pipeline.md 搂6) are a **fallback only** 鈥?for when credits can't cover the portrait
chain 鈥?and shipping them must be called out to the user, never silent. If the user chose
desktop-only, skip this 鈥?the engine still hardens phone scrubbing regardless
(seek-coalescing, iOS priming), so the page degrades gracefully rather than breaking.

---

## Step 7 鈥?Assemble the page

Copy `references/scrub-engine.js` (and, if you want a fully standalone page, the tiny
`references/index-template.html`) into the user's project 鈥?or adapt into their
...
mountScrollWorld(document.getElementById('world'), {
  ...
  sections: [
    { id:'farm', label:'The Farms', still:'assets/farm.webp',
      ...
      body:'鈥?, tags:['Single-origin','Hand-picked'] },
    // 鈥ne per section; last may carry a `cta`
  ],
  connectors:       ['assets/vid/conn1.mp4','assets/vid/conn2.mp4',   /* 鈥?length = sections-1 */],
  connectorsMobile: ['assets/vid/conn1-m.mp4','assets/vid/conn2-m.mp4' /* 鈥?same length; mobile opt-in only */],
});
```

The engine handles: the ordered dive/connector chain, scroll鈫抍urrentTime with rAF
...
`diveScroll` for that scene (more scroll = longer dwell) and `linger` (0鈥?, keep 鈮?0.6)
remaps time so the camera settles mid-scene 鈥?exactly while the copy peaks 鈥?then speeds
...
`--sw-bg`, `--sw-ink`, 鈥? 鈥?the visual identity comes from the generated clips, so the
chrome stays quiet. See the header of `scrub-engine.js` for the full config + CSS vars.

**On phones the engine adapts automatically** (coarse pointer or 鈮?60px): it serves
`clipMobile` / `connectorsMobile` when present, **coalesces seeks** (never queues a new
`currentTime` while the decoder is still seeking 鈥?this is what stops a fast flick from
...
insets so copy clears the notch/home indicator. All of this hardening is on by default 鈥?no config needed. The `clipMobile`/`connectorsMobile` encodes are the opt-in part
...
## Step 8 鈥?QA the seams (don't skip)
...
- Screenshot at scroll positions just before and just after each seam. The two frames
  ...
  PSNR 鈥?at 720p/1080p a correctly frame-locked seam can read ~18鈥?5 dB from detail
...
- **Mobile 鈥?full checklist only if the user opted into the mobile version (Step 1.6).**
  For a desktop-only build, just sanity-check a phone viewport once: page loads, still
  posters show, nothing overlaps 鈥?the engine's hardening covers graceful degradation.
  For the mobile build (do this on a real phone or an emulated one, portrait + landscape):
  - Emulate a phone viewport **with CPU throttled 4鈥?脳** and scroll fast 鈥?the clip should
  ...
  - Confirm the first scene shows immediately (its still is the poster) and the video takes
    over the instant you scroll 鈥?no blank/black scene (the iOS priming fix). Test iOS Safari
  ...
  - Verify the `-m.mp4` variant is actually served on mobile (Network panel), and the
    ...
    (`videoWidth < videoHeight` 鈥?not a downscaled 16:9 file), and the `stillMobile`
    posters must be served and match each portrait clip's first frame (no
    landscape鈫抪ortrait flash when the video paints).
  - Slowly scroll so the URL bar collapses 鈥?the page must **not jump** (height-only resizes
    are ignored on touch). Rotate the device 鈥?layout should recompose cleanly.
  - Only if the crop **fallback** shipped (no credits for the portrait chain): portrait
    crops a 16:9 clip to its centre 鈥?confirm the focal subject still reads, and remind
...
- **Seam pop** 鈫?connector endpoints were the diorama stills, not the neighbouring
  clips' actual frames. Always extract real frames (Step 5).
- **Seam stutter / camera "jumps backward"** 鈫?even with frame-matched seams, if the
  ...
  architecture A (one continuous forward take 鈥?legs chained from actual last frames, no
  pull-back, no `--end-image`); see Step 4.
- **Frozen video / stuck at frame 0** 鈫?`seekable=[0,0]`; the host isn't serving byte
  ranges. Use blob URLs (engine does).
- **Huge files** 鈫?you used all-intra. Use `-g 8` + blob instead.
- **Soft / low quality** 鈫?you downscaled or over-compressed. Encode native 1080p,
  crf 鈮?20, add `unsharp`. Video is inherently softer than the stills 鈥?keep the stills
  as the lite fallback for max fidelity.
- **Concurrent gens 503 / "not_enough_credits" race** 鈫?transient when many launch at
  once; re-roll the individual failure, it's not really out of credits (verify with
  `higgsfield workspace list`).
- **NSFW false-positives (Seedance `status "nsfw"`)** 鈫?the video content filter flags
  ...
  wording and partly the reference frames. Fixes, in order: (1) re-roll 鈥?it's often
  non-deterministic and passes on the 2nd鈥?rd try; (2) strip trigger words and add
  "empty, unoccupied, no people, no figures, architectural, tasteful"; (3) regenerate
  just that clip on **`kling3_0`** with the same start/end frames 鈥?a different
  ...
  behind a crossfade that usually beats option (4): set the connector slot to `null` 鈥?  the engine crossfades that seam directly (optional connectors), so the page still
  completes. Budget extra credits/time for these re-rolls on interiors/real-estate content.
- **Dark / custom theme** 鈫?the engine wraps its default tokens in `@layer sw`, so a
  ...
  dark and `--sw-ink` light 鈥?the copy scrim and title shadow follow `--sw-bg` automatically.
- **Phone scrub stutters / freezes on a fast flick** 鈫?the 1080p master is too heavy for a
...
- **Blank / black scene on iOS (desktop was fine)** 鈫?an iOS Safari quirk: a muted video that
  was never played won't paint a seeked frame. The engine fixes this by keeping the still as a
  poster until the clip paints and priming each video on first touch 鈥?so **don't** hide the
  still on `loadedmetadata` or strip the `playsinline`/`muted` attributes if you adapt the
  engine into a framework.
- **Page jumps while scrolling on mobile** 鈫?something is re-running layout on the URL-bar
...
- **Copy hidden behind the URL bar / notch on mobile** 鈫?use the engine's safe-area-aware
...
- **Portrait crops the scene** 鈫?a 16:9 clip on a tall phone shows only its centre 鈥?which
  is why the mobile version is the native 9:16 chain (搂6b), never the crop. If you're seeing
...
- **`--generate-audio` errors on seedance** 鈫?omit it; mute in HTML and `-an` on encode.
- **Kling rejects your flags** 鈫?`kling3_0` has **no `--resolution` param** (don't pass
  one; encode at whatever native res ffprobe reports) and **sound defaults on** 鈥?pass
  `--sound off`. Duration default is 5; legs/dives want 10.
- **Seam pop only where you "saved credits"** 鈫?you swapped models mid-chain, or used a
  ...
  seamless. (Any model with reference-only inputs can't hold a seam at all 鈥?Step 4.)
- **WaveSpeed stills come back wrong-shaped / 4xx** 鈫?pipeline.md 搂7 uses
  `openai/gpt-image-2` (the same model Higgsfield used, just via WaveSpeed's
  gateway 鈥?schema confirmed: `prompt`/`aspect_ratio`/`resolution`/`quality`). If
  you swap it for a different still model, re-verify the field names first 鈥?don't
  assume any new model's fields match this one.
- **WaveSpeed connector looks like a plain re-render, not a handoff** 鈫?confirm the
  field is actually `last_image`, not `end_image` 鈥?that's Kling's field name on
  the same platform (`kwaivgi/kling-v3.0-std/image-to-video`), not Seedance's. The
  two models are NOT interchangeable request bodies; swapping `VMODEL`-style
  without swapping the body shape silently drops the end-frame constraint and the
  seam pops.
- **Monid seedance rejects inline images** 鈫?"Must be a public https:// URL or an
  asset://<id> reference": frames go through the free `sfs` file system
  (put 鈫?`curl -T` 鈫?cat 鈫?signed URL; pipeline.md 鈫?Monid backend), never base64.
  Quirk: `/put` echoes back `home/<path>`, but `/cat` and `/ls` want the **original
  relative path** you gave `/put` 鈥?using the echoed path 404s.
- **Monid clip wrong aspect** 鈫?the `ratio` default is adaptive and follows the input
  image (a 3:2 still 鈫?a 4:3-ish video). Pass `--ratio` 鈥?`16:9` desktop, `9:16`
  mobile chain 鈥?explicitly on every chained clip.
- **Monid CLI "Polling timed out after 120s"** 鈫?only the local wait died; the run
  continues server-side. Re-poll with `monid runs get -r <runId> -w 120` (find the id
  in `monid runs list`). Result URLs expire (~24鈥?8 h) 鈥?download immediately.
- **Monid minimax drops the image when a prompt is present** 鈫?`prompt` +
  ...
  Until the wrapper is fixed, that endpoint can't chain 鈥?use Monid's seedance-2.0.
  (The model itself is fine 鈥?the same prompt+image via Higgsfield `minimax_hailuo`
  frame-locks.)
- **Monid billing surprises** 鈫?matrix/token-priced endpoints bill by selector match:
  ...
  also bounce as an HTML error page ("Unexpected token '<'") 鈥?another reason frames
  travel by sfs URL.
- **Monid schema changed since last build** 鈫?it happens (seedance was t2v-only until
...
- **Codex stills hang at "Reading additional input from stdin..."** 鈫?parallel
  ...
  started). Always append `< /dev/null` to every backgrounded `codex exec` 鈥?the
  pipeline's `gen_still_codex` has it; keep it if you adapt the command.
- **White-box scenes** 鈫?`gpt_image_2` returns a solid bg; either match the page bg to it
  or knock it out (Step 3).
- **bash 3.2** on macOS 鈫?no associative arrays in scripts.
- **Connector grabs the wrong scene's frames** (or errors on a frame that doesn't exist
  yet) 鈫?the array loop ran in **zsh** (macOS default interactive shell), where arrays are
  1-indexed, not bash's 0-indexed. Keep every array-driven chain step in a `#!/bin/bash`
  script run via `bash script.sh` 鈥?never inline array loops in the interactive shell.

## References

- `references/bootstrap.py` - first-run setup for the default WaveSpeed path: checks
  dependencies, installs ffmpeg when possible, asks for/stores `WAVESPEED_API_KEY`, and
  verifies auth against the correct API host.
- `references/prompts.md` 鈥?the intake checklist, style-preamble pattern, and every
  prompt template (scene still, dive, connector) with fill-in slots.
- `references/pipeline.md` 鈥?copy-paste batch scripts for the whole run (generate 鈫?  extract frames 鈫?connectors 鈫?encode 鈫?mobile encode), bash-3.2-safe.
- `references/scrub-engine.js` 鈥?the portable, config-driven scrub engine (builds DOM +
...
- `references/index-template.html` 鈥?a minimal standalone page that mounts the engine.
- `references/knockout.py` 鈥?border-connected background knockout for floating scenes.
