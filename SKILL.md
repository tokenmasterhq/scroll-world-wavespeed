...
1. **Backend + billing — pick before anything else, and ask which audience this
   build is for if it isn't obvious.** Two supported paths, both roster-qualified
   (Step 4). **WaveSpeedAI is the default/primary path for this skill** — reach for
   ...
   - **WaveSpeedAI — the default path, incl. mainland-China audiences** (pipeline.md
     §7): needs only `curl`, `ffmpeg`/`ffprobe`, and `python3` (all near-universal —      no CLI install, no OAuth login, no VPN to reach the API or to pay: card or RMB
     via Alipay/WeChat) plus a `WAVESPEED_API_KEY`. `jq` is used if present but is
     **not required** — pipeline.md §7's helpers auto-fall-back to a bundled python3
     ...
     *and* connectors — Higgsfield/Monid are not needed on this path, full stop; do
     not tell a user on this path they need to install either CLI.
   - **Monid + Higgsfield — alternate path, international/USD builds only**
     (pipeline.md §7b): check `monid --version`, `monid keys list`, `monid balance`
     (billed per clip in USD; Step 1.7 has the numbers; a 1080p N=6 chain ≈ $27).
     Higgsfield CLI is required on this path (see bullet 2) — but *only* on this
     path.
2. **Higgsfield CLI — only required on the Monid path (skip entirely on WaveSpeed).**
   ...
   ask the user to run `higgsfield auth login` (interactive OAuth — you cannot run
   it) and, if needed, `higgsfield workspace set <id>`. Confirm credits cover the
   stills (~N image gens) — plus `(2N-1)` video gens if the chain falls back here.
3. **ffmpeg / ffprobe** on `$PATH` (frame extraction + encoding) — required on
...
6. Caveats: macOS ships **bash 3.2** (no `declare -A`); don't use associative arrays in
   scripts. Higgsfield generations take **3–5 min each** — always run them detached
...
2. **Brand kit** — offer three paths, pick one:
   ...
   Capture **4—  named hex values**, a display name, and a tone word or two.
...
5. **The journey (sections)** — the ordered scenes the camera flies through. Propose a
   set derived from the subject's own value chain and let the user edit. 5—  works well.
   Boba example: farms → pearl kitchen → flagship shop → delivery → community plaza →    the hero product. Each section needs: a short subject description (what's IN the
...
7. **Budget — engines shown by cost, decided before anything renders.** Present the
   ...
   - **Calibrate costs, don't guess.** The CLI exposes no pricing and plans differ.
     ...
     video ≈40–45 credits, still ≈15.) A real `not_enough_credits` mid-run is
...
- **Codex stills variant** (if chosen at Step 1.7 — subscription-billed, zero
  ...
  ~1—  min per image; run a few in parallel, not all N at once — and keep the
...
probe: **`minimax_hailuo`** (Hailuo-2.3, ~6 credits per 768p/6s clip vs 22— 2 for the
...
- Default to `bytedance/seedance-2.0`, rendered through **WaveSpeedAI by default** (per-clip USD —   next section) with Higgsfield credits as the fallback biller (Step 0.1/1.6); honor
...
3. **Bill-check every clip**: cost is token-priced
   (`w × h × 24 × sec / 1024` at $7— .7/1M); read `cost.value` off each run.
...
- Params (seedance): `--mode std --resolution 1080p --aspect_ratio 16:9 --duration 8`.
  ...
  Do **not** pass `--generate-audio` (it errors on seedance; audio is wasted anyway —   you'll mute).
...
connector, encoded **720 wide (`scale=720:-2`), `-g 4`** (more keyframes = cheaper seeks — phone decoders' seek cost scales with GOP length), crf 23 — wired as `clipMobile` /
...
`diveScroll` for that scene (more scroll = longer dwell) and `linger` (0— , keep ≈ 0.6)
...
**On phones the engine adapts automatically** (coarse pointer or ≈ 60px): it serves
...
insets so copy clears the notch/home indicator. All of this hardening is on by default — no config needed. The `clipMobile`/`connectorsMobile` encodes are the opt-in part
...
- **NSFW false-positives (Seedance `status "nsfw"`)** → the video content filter flags
  ...
  behind a crossfade that usually beats option (4): set the connector slot to `null` —   the engine crossfades that seam directly (optional connectors), so the page still
...
- **WaveSpeed stills come back wrong-shaped / 4xx** → pipeline.md §7 uses
  `openai/gpt-image-2` (the same model Higgsfield used, just via WaveSpeed's
  gateway — schema confirmed: `prompt`/`aspect_ratio`/`resolution`/`quality`). If
  you swap it for a different still model, re-verify the field names first — don't
  assume any new model's fields match this one.
- **WaveSpeed connector looks like a plain re-render, not a handoff** → confirm the
  field is actually `last_image`, not `end_image` — that's Kling's field name on
...
- `references/pipeline.md` — copy-paste batch scripts for the whole run (generate →   extract frames → connectors → encode → mobile encode), bash-3.2-safe.
